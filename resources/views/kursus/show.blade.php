    ...
    <div class="col-lg-8">
        <div class="card shadow-sm">
            <div class="course-header">
                <h1 class="course-title">{{ $kursus->nama_kursus }}</h1>
                <div class="course-meta">
                    <div class="meta-item">
                        <i class="bi bi-person-badge"></i>
                        <span>{{ $kursus->pengajar->nama_pengajar }}</span>
                    </div>
                    <div class="meta-item">
                        <i class="bi bi-people-fill"></i>
                        <span>{{ $kursus->peserta->count() }} Peserta</span>
                    </div>
                    <div class="meta-item">
                        <i class="bi bi-clock"></i>
                        <span>{{ $kursus->durasi ?? '8 Minggu' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="course-content">
                ...
                <div class="btn-group-custom">
                    <a href="{{ route('kursus.edit', $kursus) }}" class="btn btn-warning">
                        <i class="bi bi-pencil"></i> Edit Kursus
                    </a>
                    <form action="{{ route('kursus.destroy', $kursus) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                ...
                <a href="{{ route('kursus.index') }}" class="btn btn-secondary mt-3">
    ...
    <div class="col-lg-4">
        ...
        <div class="card shadow-sm mb-4" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
            <div class="card-body text-center">
                ...
                @else
                <p class="mb-4">Bergabunglah dengan {{ $kursus->enrolledUsers->count() }} peserta lainnya</p>

                @auth
                @if(!auth()->user()->isAdmin())
                <form action="{{ route('kursus.enroll', $kursus) }}" method="POST">
                    @csrf
                ...
                <form action="{{ route('login') }}" method="GET">
                    <input type="hidden" name="intended" value="{{ route('kursus.show', $kursus) }}">
                    <button type="submit" class="btn btn-light btn-lg w-100 fw-bold">
                        <i class="bi bi-box-arrow-in-right"></i> Login untuk Mendaftar
                    </button>
                </form>
                <small class="d-block mt-2">Belum punya akun? <a href="{{ route('register') }}" class="text-white fw-bold">Daftar disini</a></small>
                @endauth
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="instructor-card">
                    <div class="instructor-avatar">
                        {{ strtoupper(substr($kursus->pengajar->nama_pengajar, 0, 1)) }}
                    </div>
                    <div>
                        <h6 class="mb-1">{{ $kursus->pengajar->nama_pengajar }}</h6>
                        <p class="text-muted mb-0" style="font-size: 0.9rem;">
                            <i class="bi bi-award"></i> {{ $kursus->pengajar->keahlian }}
                        </p>
                        <small class="text-muted">
                            <i class="bi bi-envelope"></i> {{ $kursus->pengajar->email }}
                        </small>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <h5 class="card-title mb-3">
                    <i class="bi bi-people-fill"></i> Daftar Peserta ({{ $kursus->enrolledUsers->count() }})
                </h5>

                @if($kursus->enrolledUsers->count() > 0)
                <ul class="peserta-list">
                    @foreach($kursus->enrolledUsers as $user)
